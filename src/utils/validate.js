 ...
 * @Last Modified time: 2021-01-21 16:14:16
 */
...
let validNonNegativeInteger = (rule, value, callback) => {
    let regexp = /^[1-9]\d*$|0$/;
    if (!value) {
        callback(new Error(`请填写数字类型`));
    } else {
        if (!regexp.test(value) || String(value).indexOf('-') !== -1) {
...
export {
    ...
    validNonNegativeInteger// 校验非负整数
};
